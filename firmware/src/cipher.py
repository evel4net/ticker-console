...
class Cipher(object):
    ...
    def get_shared_secret(self, other_pub_hex: str):
    ...
    def aes_ctr_mode(self, iv: bytes, text: bytes) -> bytes:
        cipher = ucryptolib.aes(self.__aes_key, 1) # ECB mode
    ...
    def encrypt_text(self, plain_text: bytes) -> (bytes, bytes, bytes):
        iv = uos.urandom(16) # counter block
        cipher_text = self.aes_ctr_mode(iv, plain_text)

        hmac_tag = hmac.new(self.__aes_key, iv + cipher_text, hashlib.sha256).digest()

        return iv, cipher_text, hmac_tag

    def decrypt_text(self, iv: bytes, cipher_text: bytes, hmac_tag: bytes) -> bytes:
        hmac_expected = hmac.new(self.__aes_key, iv + cipher_text, hashlib.sha256).digest()

        if hmac_expected != hmac_tag:
            raise UnauthorizedAccessException("HMAC tag not corresponding.")

        plain_text = self.aes_ctr_mode(iv, cipher_text)

        return plain_text

    def encrypt_response(self, response: dict) -> dict:
